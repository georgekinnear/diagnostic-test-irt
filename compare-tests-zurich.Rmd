...
test_scores_pre <- scores_pre %>%
  ...
  # relabel the columns to use z rather than A for the question names
...
test_versions <- read_csv("data-eth/eth-metadata.csv") %>% 
  mutate(
    pre = str_replace(pre, "A", "z"),
    post = str_replace(post, "B", "Z")
  ) %>% 
  mutate(
    label = case_when(
      !is.na(pre) & !is.na(post) ~ str_glue("{pre}_{post}"),
      !is.na(pre) & is.na(post) ~ str_glue("{pre}_Z0"),
      is.na(pre) & !is.na(post) ~ str_glue("z0_{post}"),
    ),
    item_num = row_number()
  ) %>% 
  mutate(
