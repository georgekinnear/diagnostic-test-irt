...
These values are also saved to the file `data-eth/OUT_2pl-results.csv`.
...
```{r}
unchanged_items <- test_versions %>% filter(!is.na(pre), !is.na(post)) %>% pull(item_num)
info_retained <- areainfo(fit_2pl, c(-4,4), which.items = unchanged_items)
info_retained %>% gt()
```
...
```{r}
removed_items <- test_versions %>% filter(is.na(post)) %>% pull(item_num)
info_removed <- areainfo(fit_2pl, c(-4,4), which.items = removed_items)
info_removed %>% gt()
```
...
```{r}
added_items <- test_versions %>% filter(is.na(pre)) %>% pull(item_num)
info_added <- areainfo(fit_2pl, c(-4,4), which.items = added_items)
info_added %>% gt()
```
...
The mean information in the `r info_removed$nitems` removed questions was `r info_removed$TotalInfo/info_removed$nitems`.

The mean information in the `r info_added$nitems` added questions was `r info_added$TotalInfo/info_added$nitems`.

Thus, relative to the expected information if we had simply picked `r info_added$nitems` out of the `r info_removed$nitems` removed items to retain, the new questions have added `r info_added$nitems*(info_added$TotalInfo/info_added$nitems - info_removed$TotalInfo/info_removed$nitems)` to the information.

This is very small relative to the total information in the test. 
