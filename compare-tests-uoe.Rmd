...
test_scores_pre <- scores_pre %>%
  ...
  rename_with(.cols = matches("^Q\\d"), .fn = ~ str_replace(., "Q", "e")) %>% 
...
test_scores_post <- scores_post %>%
  ...
  rename_with(.cols = starts_with("Q"), .fn = ~ str_replace(., "Q", "E") %>% str_remove(., "\\.\\.5\\.00")) %>% 
...
# e.g. e4_E7 means the item was e4 on version A and E7 on version B
# (e0 and E0 are used to indicate the question did not appear on that version)
test_versions <- read_csv("data-uoe/edinburgh-diagtest-metadata.csv") %>% 
  mutate(
    pre = str_replace(pre, "A", "e"),
    post = str_replace(post, "B", "E")
  ) %>% 
  mutate(
    label = case_when(
      !is.na(pre) & !is.na(post) ~ str_glue("{pre}_{post}"),
      !is.na(pre) & is.na(post) ~ str_glue("{pre}_E0"),
      is.na(pre) & !is.na(post) ~ str_glue("e0_{post}"),
    ),
    item_num = row_number()
  ) %>% 
  mutate(
