...
ggsave("uoe_3factor.pdf", units = "cm", width = 12, height = 8, dpi = 300,
       plot = load3_plot)
```
...
```{r save-results}
tidy_gpcm %>% 
  write_csv("data-uoe/OUT_gpcm-results-pre-only.csv")
```
...
plt <- plot(fit_gpcm, type = "infoSE", main = "Test information")
...
ggplot(plt_data, aes(x, y)) + 
  geom_line() + 
  labs(
    title = "2PL - Test information",
    #x = expression(theta),
    x = "theta",
