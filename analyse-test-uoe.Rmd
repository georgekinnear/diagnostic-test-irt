...
item_info <- read_csv("data-uoe/edinburgh-diagtest-metadata.csv") %>% 
  ...
  mutate(question = str_replace(question, "A", "e"))
...
test_scores <-
  ...
  filter(!is.na(Total)) %>%
  rename_with(.cols = starts_with("Q"), .fn = ~ str_replace(., "Q", "e")) %>% 
  select(year, AnonID, Total, starts_with("e"))
...
test_scores <- test_scores_unfiltered %>% 
  ...
  ungroup() %>% 
  rowwise() %>% 
  mutate(zeros_in_easiest_5 = sum(e11==0, e12==0, e13==0, e16==0, e17==0)) %>% 
...
The second factor is dominated by the two chain rule questions (e16 and e17), along with e18 which is a routine definite integral, suggesting this factor is related to routine calculus computations.
...
residuals %>%
  ...
  as_tibble() %>% 
  pivot_longer(cols = !item1, names_to = "item2", values_to = "Q3_score") %>% 
  filter(abs(Q3_score) > 0.2) %>% 
  filter(parse_number(item1) < parse_number(item2)) %>%
  gt()
```

Items e16 and e17 are on the chain rule (e.g. differentiating $\cos(4x^2+5)$ and $(3x^2-8)^3$ respectively), so it is perhaps unsurprising that students' performance on these items is not entirely independent.
...
highlight_removed_items <- expected_scores %>% 
  mutate(highlight_item = item %in% c("e2", "e8", "e11")) %>% 
