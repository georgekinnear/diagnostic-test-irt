...
author: "Mine Çetinkaya-Rundel, George Kinnear"
date: '2020-09-04'
output:
  html_document:
    toc: yes
...
(A) Chito's approach
Again, this does not run and would need lots of tweaking to get it working. See source in https://github.com/mine-cetinkaya-rundel/irt-eth/blob/master/MDT2018_FINAL/code/R_code_export/IMPRV.r and the files beginning f_ and h_ in https://github.com/mine-cetinkaya-rundel/irt-eth/tree/master/MDT2018_FINAL/code
```{r eval=FALSE, include=FALSE}
...
# NB - this issue seems related: https://github.com/philchalmers/mirt/issues/163 (and the relevel_items gives a more tidyverse way of re-levelling, though it doesn't retain knowledge of the correspondence between old and new scores)
...
item_data_relabelled <- my_recategorise(data = mdt_all_data, 
                                        MAX_MARKS = rep(5, 20))
head(item_data_relabelled$data_recat$Q1)
item_data_relabelled$MARK_CATEGORIES$Q1
head(mdt_all_data %>% drop_na %>% select(Q1))
```
